...
const PoolCard: React.FC<IProp> = ({ pool }) => {
  const stakingTokenContract = useERC20(
    pool.stakingToken.address ? getAddress(pool.stakingToken.address) : "",
  );
  ...
  return (
    <Flex flexDirection="column" mr={["8px", 0]} key={pool.sousId}>
      ...
      <p>{`Your Staked On Pool:  ${
        pool.userData
          ? getFullDisplayBalance(
              new BigNumber(pool.userData.stakedBalance.toString()),
            )
          : 0
      }`}</p>
      <p>{`Your Pending Reward On Pool:   ${
        pool.userData
          ? getFullDisplayBalance(
              new BigNumber(pool.userData.pendingReward.toString()),
            )
          : 0
      }`}</p>
      {!account ? (
            ...
            <Button
              onClick={async () => {
                try {
                  ...
                  console.log(
                    `Staked. Your ${pool.stakingToken.symbol} funds have been staked in the pool!`,
                  );
                } catch (e) {
                  console.error(
                    "Canceled. Please try again and confirm the transaction.",
                    e,
                  );
                }
              }}
